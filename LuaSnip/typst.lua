...
local function in_text()
  return not ts_has_root("math") and not ts_has_root("comment") and not ts_has_root("raw_blck")
end
local function in_math()
  return ts_has_root("math")
end
local function in_code()
  return ts_has_root("raw_blck")
end
local function in_comment()
  return ts_has_root("comment")
end
...
return {
  ...
  sm({ trig = "OO", name = "Empty Set" }, { t("diameter ") }),
  ...
  sm({ trig = "oo", name = "Infinity", wordTrig = false }, { t("infinity ") }),
  ...
  sm(
    { trig = "(%a)vec", name = "Vectors", regTrig = true },
    fmt([[arrow({}) ]], {
      f(function(_, snip)
        return snip.captures[1]
      end)
